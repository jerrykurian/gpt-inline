...
chrome.runtime.onMessage.addListener(function (response, sender, sendResponse) {
  ...
  switch (response.type) {
    ...
    case "List Models Response":
      handleListModelsResponse(response);
      break;
  }
});
...
function showApiKeyModal() {
  loadApiKey((gptApiKey) => {
    ...
    if (!keyContentAdded) {
      /* let gptMessage = document.createElement('gpt-message');
                // Add gpt-modal modal-sheet position-top position-right bg-body-secondary to the classList of gptMessage
                gptMessage.classList.add('gpt-modal');
                gptMessage.classList.add('modal-sheet');
                gptMessage.classList.add('position-top');
                gptMessage.classList.add('position-right');
                gptMessage.classList.add('bg-body-secondary');


                $(document.body).append(gptMessage);*/
...
function showConfirmation(completion) {
  fetch(chrome.runtime.getURL("/pages/confirmation.html"))
    .then((r) => r.text())
    .then((html) => {
      if ($("#gpt-confirmation-modal").length > 0)
